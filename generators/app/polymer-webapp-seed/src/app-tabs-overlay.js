...
$_documentContainer.innerHTML = `<dom-module id="app-tabs-overlay">
...
class AppTabsOverlay extends Element {
  ...
  _targetChanged(newTarget, oldTarget) {
    if (!this._transitionsInFlight.length) {
      if (this._lastTarget) {
        this._lastTarget.classList.remove('app-tabs-overlay-static-above');
      }
  ...
  _moveComplete() {
    if (this._lastTarget !== this.target) {
      ...
      if (this._lastTarget) {
        this._lastTarget.classList.add('app-tabs-overlay-static-above');
      }
...
customElements.define(AppTabsOverlay.is, AppTabsOverlay);